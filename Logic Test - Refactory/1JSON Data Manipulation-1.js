...
//1. Find all purchases made in March
const marpcs = data.filter((item) => item.month === "2018-03");

console.log(marpcs);

//2. Find all purchases made by Anis
function allpcs(name) {
  const fd = data.filter((items) => item.items === name);
  const ttlpcs = fd.reduce((acc, items) => acc + item.items, 0);
  return ttlpcs;
}

console.log(allpcs("Anis"));
